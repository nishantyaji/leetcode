...
class MaxSortedDict:
    ...
    def add_elem(self, key: int):

        if key not in self.cntr:
            self.cntr[key] = 0
            val = 0
        else:
            val = self.cntr[key]
            self.values_list.remove(val)
        self.values_list.add(val + 1)
        self.cntr[key] = val + 1

    def rem_elem(self, key: int):
        if key in self.cntr:
            val = self.cntr[key]
            self.values_list.remove(val)
            if val > 1:
    ...
    def __init__(self, arr: list[int]):
        self.cntr = dict()
        self.values_list = SortedList()
        if arr:
            for a in arr:
                self.add_elem(a)

    def max_freq(self):
        return self.values_list[-1] if self.values_list else None
